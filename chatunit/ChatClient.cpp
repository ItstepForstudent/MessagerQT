...
ChatClient:: ChatClient(const QString& strHost,
                   int            nPort,
                   QWidget*       pwgt /*=0*/
) : QWidget(pwgt)
...
void ChatClient::slotSendToServer()
{
    ...
    QDataStream out(&arrBlock, QIODevice::WriteOnly);
    out.setVersion(QDataStream::Qt_5_11);
    QString str = "<?xml version=\"1.0\"?><request><type>GetClients</type><data><name>vasia</name></data></request>";
    out<< qint32(str.size()+1)<<str;

    /*out << quint32(0) << QTime::currentTime() << m_ptxtInput->text();
